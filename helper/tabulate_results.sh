...
if [ ${PRS_type} != 'snpnet' ] && [ ${PRS_type} != 'vanilla' ] ; then usage ; exit 1 ; fi
...
| {
  if [ ${PRS_type} == 'snpnet' ] ; then
    awk -v FS='\t' -v OFS='\t' '{split($1, a, "/*"); print a[length(a)], "snpnet_PRS", $2, $3, $4}'
  else # vanilla PRS
    awk -v FS='\t' -v OFS='\t' '{split($1, a, "/*"); print a[length(a)], "vanilla_PRS_" a[length(a) - 1], $2, $3, $4}'
  fi
} \
| sort -k1b,1 \
