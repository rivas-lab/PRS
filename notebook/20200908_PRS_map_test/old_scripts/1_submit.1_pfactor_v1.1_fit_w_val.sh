...
echo $GBE_ID
resbatch.sh --job_cmd 'sbatch -p mrivas --qos=high_p --job-name=snpnet --output=logs/snpnet.%A.out --error=logs/snpnet.%A.err --time=2-0:00:00' --mem 60000 -c 10 --mem_mult 2 --try_n 1 --log logs/snpnet.${GBE_ID}.log --src snpnet.1_fit_w_val.sh - ${GBE_ID} $(get_family_from_GBE_ID ${GBE_ID})
