...
var_anno_df %>% filter(1e-4 <= maf, maf <= 1e-3, ld_indep) %>% pull(ID) -> rare_vars
...
geom_errorbarh(aes(xmin = BETA_1 - SE_1, xmax = BETA_1 + SE_1), alpha=.1) +
geom_errorbar( aes(ymin = BETA_2 - SE_2, ymax = BETA_2 + SE_2), alpha=.1) +
labs(title=sprintf('%s, test set', GBE_ID), x = 'BETA [SE], default run', y = 'BETA [SE], w/ PRS in covariates')

p_B_46 <- df_46 %>% filter(log10P_1 >= p_thr | log10P_2 >= p_thr ) %>% ggplot(aes(x = BETA_1, BETA_2)) + 
theme_bw() + geom_abline(slope=1, intercept=0, color='red') + geom_point() +
geom_errorbarh(aes(xmin = BETA_1 - SE_1, xmax = BETA_1 + SE_1), alpha=.1) +
geom_errorbar( aes(ymin = BETA_2 - SE_2, ymax = BETA_2 + SE_2), alpha=.1) +
labs(title=sprintf('%s, training/validation set', GBE_ID), x = 'BETA [SE], default run', y = 'BETA [SE], w/ PRS in covariates')

ggsave(
    sprintf('%s.pdf', GBE_ID),
