...
# re-submission
resbatch.sh --job_cmd 'sbatch -p mrivas --qos=high_p --job-name=snpnet --output=logs/snpnet.%A.out --error=logs/snpnet.%A.err --time=2-0:00:00' --mem 600000 -c 10 --mem_mult 1.5 --try_n 1 --log logs/snpnet.${GBE_ID}.log --src snpnet.sh - ${GBE_ID} binomial
