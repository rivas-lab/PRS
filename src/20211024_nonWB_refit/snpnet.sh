...
cat <<- EOF
	...
	Usage: $PROGNAME [options] phenotype_name pop
	  phenotype_name   The input file
...
phe_file="/scratch/groups/mrivas/ukbb24983/phenotypedata/master_phe/master.20211020.phe.zst"
project_dir="/scratch/groups/mrivas/projects/PRS/private_output/$(basename ${SRCDIR})"
results_dir="${project_dir}/${run_name}/${phenotype_name}/${pop}"
WB_fit_beta="/oak/stanford/groups/mrivas/projects/PRS/GBE_data/${phenotype_name}.tsv"
...
if [ "${refit}" == "T" ] ; then
    ...
    zstdcat ${phe_file} | awk -v OFS='\t' -v pop=${pop} '((NR>1) && ($3 == pop) && ($5 == "train" || $5  == "val")){print $1, $2}' > ${keep}
else
    results_sub_dir="1_fit_w_val"
    keep="/scratch/groups/mrivas/ukbb24983/sqc/population_stratification_w24983_20211020/ukb24983_${pop}.phe"
fi
...
$([ "${refit}" == "T" ] && echo "--split_col NULL" || echo "--split_col split_nonWB") \
${genotype_pfile} \
${phe_file} \
${phenotype_name} \
${family} \
...
run-simg.sh Rscript ${SRCDIR}/snpnet.eval.R ${phenotype_name} ${results_dir}/${results_sub_dir} ${pop} $([ "${refit}" == "T" ] && echo "refit" || echo "")
